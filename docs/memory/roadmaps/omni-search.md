...
6. **Implement dynamic search API** (done)
   ...
   - Query users, clients, roles; return capped, relevance-sorted results.
...
2. **Keyboard UX + accessibility** (next)
   - Arrow-key focus syncs with inspector.
   - Make switch interactions keyboard-safe.
   - Add `aria` roles for split-pane content.
