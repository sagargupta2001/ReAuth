...
async fn test_create_role_success() {
    ...
    let payload = CreateRolePayload { name: "admin".to_string(), description: None };
...
async fn test_create_group_success() {
    let ctx = setup_test_env().await;
    let payload = CreateGroupPayload { name: "developers".to_string(), description: None };
...
async fn test_create_group_duplicate_fails() {
    ...
    ctx.rbac_service.create_group(payload.clone()).await.unwrap();
...
async fn test_cache_is_invalidated_when_user_assigned_to_group() -> anyhow::Result<()> {
    // 1. ARRANGE
    let ctx = setup_test_env().await;
    let user = ctx.user_service.create_user("test@user.com", "pass").await?;
    let role = ctx.rbac_service.create_role(CreateRolePayload { name: "support".to_string(), ..Default::default() }).await?;
    let group = ctx.rbac_service.create_group(CreateGroupPayload { name: "support_team".to_string(), ..Default::default() }).await?;

    // Link permission -> role -> group
    ctx.rbac_service.assign_permission_to_role(TEST_PERMISSION.to_string(), role.id).await?;
    ctx.rbac_service.assign_role_to_group(role.id, group.id).await?;
    ...
    assert!(perms_before.is_empty(), "Permissions should be empty initially");

    // Prove it's in the cache by checking the adapter directly
    let cached = ctx.cache_service.get_user_permissions(&user.id).await;
    assert!(cached.is_some(), "Cache should be populated (with empty set)");

    // 3. ACT (Trigger the event)
    ctx.rbac_service.assign_user_to_group(user.id, group.id).await?;
    ...
    assert!(cached_after.is_none(), "Cache should have been invalidated by the event");
