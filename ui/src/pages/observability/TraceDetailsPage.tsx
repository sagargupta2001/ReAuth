...
export function TraceDetailsPage() {
  ...
  // Surface the trace id in the header breadcrumb, consistent with other detail pages.
  useSetBreadcrumb({ [traceId ?? '']: traceId ?? '' })

  const { data: spans = [], isError, isLoading } = useTelemetryTraceSpans(traceId)
  const primaryTrace = useMemo(() => selectPrimaryTrace(spans), [spans])
  const spanRows = useMemo(() => buildSpanRows(spans), [spans])
  const title = traceLabel(primaryTrace, traceId ?? null)
  ...
  return (
    <div className="bg-background flex h-full w-full flex-col overflow-hidden">
      <div className=" shrink-0 px-6 py-5">
        <div className="mt-2 flex min-w-0 flex-wrap items-center gap-2 text-xl">
          {primaryTrace?.method ? (
          ...
          <div className="min-w-[780px] p-6">
            <div className="mb-4 flex items-center justify-between gap-4">
              <div>
                ...
                <p className="text-muted-foreground text-xs">{text.spanCount}</p>
              </div>
            </div>
