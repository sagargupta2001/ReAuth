...
import { useAddUserPhoneNumber } from '@/features/user/api/useUserPhoneNumbers.ts'
...
const addPhoneNumberFormSchema = z.object({
  phone_number: z.string().trim().min(1, { message: 'Phone number is required' }),
  is_primary: z.boolean(),
  is_verified: z.boolean(),
})

type AddPhoneNumberFormValues = z.infer<typeof addPhoneNumberFormSchema>

interface AddPhoneNumberDialogProps {
  userId: string
}

export function AddPhoneNumberDialog({ userId }: AddPhoneNumberDialogProps) {
  const [open, setOpen] = useState(false)
  const mutation = useAddUserPhoneNumber(userId)

  const form = useForm<AddPhoneNumberFormValues>({
    resolver: zodResolver(addPhoneNumberFormSchema),
    defaultValues: { phone_number: '', is_primary: false, is_verified: false },
  })
  ...
  const onSubmit = (values: AddPhoneNumberFormValues) => {
    mutation.mutate(
      {
      ...
      },
      {
        ...
        onError: (error) => {
          if (error instanceof ApiError) {
            form.setError('phone_number', { type: 'server', message: error.message })
          }
        },
      },
    )
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogTrigger asChild>
        <Button variant="highlight" size="clear" className="items-center gap-1 self-stretch">
          ...
          <span className="flex items-center text-sm leading-none font-medium">Add phone</span>
        </Button>
      </DialogTrigger>
          ...
          <div className="grid gap-4 px-6 pb-6">
            ...
            <div className="mt-2 flex items-start space-x-3">
              ...
              <div className="-mt-0.5 grid gap-1.5 leading-none">
                <label
                  htmlFor="phone_is_primary"
                  className="cursor-pointer text-[14px] leading-none font-medium"
                >
                  Set as primary
                </label>
                <p className="text-muted-foreground mt-1 text-[13px]">
                  Promote this number to the account&apos;s primary phone immediately.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start space-x-3">
              <Checkbox
                id="phone_is_verified"
                checked={form.watch('is_verified')}
                onCheckedChange={(checked) => form.setValue('is_verified', checked as boolean)}
              />
              <div className="-mt-0.5 grid gap-1.5 leading-none">
                <label
                  htmlFor="phone_is_verified"
                  className="cursor-pointer text-[14px] leading-none font-medium"
                >
                  Mark as verified
                </label>
                <p className="text-muted-foreground mt-1 text-[13px]">
                  Skip verification and treat this phone number as already confirmed.
                </p>
              </div>
            </div>
          </div>
